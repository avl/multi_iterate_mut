...
struct MultiIterateContext2<'a,T,
    A1,FA1:for <'r> FnOnce(&'r mut A1),
    A2,FA2:for <'r> FnOnce(&'r mut A2)
> {
    chunk_size:usize,
        ...
        pub fn $name<L: $fntype + Send+Sync>(&mut self, pool: &mut Pool, l:L) {
...

            {
                ...
                pool.scoped(move |scope| {
                    for (datachunk_items, aux_mutators) in &mut selfdata.chunks_mut(chunk_size)
...
#[inline]
fn run_aux<A1:Send,FA1:for <'r> FnOnce(&'r mut A1) + Send>(mut aux_and_closures: (&mut Vec<A1>, Vec<FuncThreadData<FA1>>) , aux_data: &mut AuxData) {
...
> MultiIterateContext2<'a,T,A1,FA1,A2,FA2> {
    ...
    #[inline(always)]
    fn modify_aux0(&mut self, aux_index: usize, f:FA1) {
        let pass = aux_index / self.chunk_size;
        (self.aux_mutator_closures.0.data)[pass].push((aux_index, f));
    }
    #[inline(always)]
    fn modify_aux1(&mut self, aux_index: usize, f:FA2) {
        let pass = aux_index / self.chunk_size;
...

                {
                    test.modify_aux(primary_value,
                                    #[inline(always)]
                                        ...
                                        aux1.item =((primary_value as f64*0.001).cos().cos().cos()*100.0) as usize;
                                    });
                }
    ...
    fn benchmark_simple_unithread(bench:&mut Bencher) {
        ...
        bench.iter(move||{
            ...
            for (aux1,data) in &mut aux1.iter_mut().zip(data.iter_mut()) {
                aux1.item = ((data.item as f64*0.001).cos().cos().cos()*100.0) as usize;
            }
        });
    }
